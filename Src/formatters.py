...
def format_contacts(book):
    ...
    for record in book.values():
        ...
        emails = ", ".join(email.value for email in getattr(record, 'emails', [])) if hasattr(record, 'emails') else "-"
        addresses = ", ".join(addr.value for addr in getattr(record, 'addresses', [])) if hasattr(record, 'addresses') else "-"
...
def format_notes(notebook):
    ...
    headers = [Fore.MAGENTA + "Title" + Style.RESET_ALL, Fore.MAGENTA + "Text" + Style.RESET_ALL]
    table = []
